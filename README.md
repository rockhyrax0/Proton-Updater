...
- **Live progress notifications.** A single popup that updates in place with a real progress bar on KDE and GNOME. Degrades cleanly to one static notification on older `notify-send`, and to silence with `NO_NOTIFY=1`.
